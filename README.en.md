...
Echo Agent is a self-hosted AI Agent. On top of the standard Agent Loop it adds two layers most agents are missing: **cognitive memory that survives across sessions**, so the agent remembers conversation context, user preferences and task experience; and **self-evolution driven by real execution traces**, so the agent's skills keep improving as it runs, instead of being frozen at release time.
