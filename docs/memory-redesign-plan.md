...
- 有硬字符预算;溢出是「该策展整理」的信号,不是悄悄丢弃(借 openclaw 纪律)。
...
- **1.3** Core 区字符预算 + 溢出策展信号(借 hermes 有界 + openclaw「溢出=该整理」)。
...
- **2.2** 审查写入前做威胁扫描(借 hermes `_scan_memory_content`):
...
## 五、不做什么(明确排除)
- 不引入 openclaw 的 dreaming/REM/向量知识库/wiki —— 对单人助理是过度设计。
- 不照搬 hermes 的物理双文件 —— echo-agent 已有 USER/ENV 类型,逻辑分层即可。
