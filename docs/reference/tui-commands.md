...
`echo-agent cli` 默认使用保留原生 scrollback 的 inline 界面，
`echo-agent cli --tui` 使用全屏 Textual 界面。两种界面共用下列命令及
...
无参数时显示三个分区的当前状态。默认为“思考=折叠、工具=精简、
状态=隐藏”；工具精简会隐藏成功的只读调用，但任何失败始终可见。设置只影响
之后到达的过程信息，不影响最终回答或 JSON 审计导出。
