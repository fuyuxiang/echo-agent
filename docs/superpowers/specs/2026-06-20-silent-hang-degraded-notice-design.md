...
## 2. 横向对比(openclaw / hermes-agent)

两个同类项目都遇到过同型故障并已专门修复(代码注释引用对应 issue:openclaw #26905/#36142,hermes #18765/#24912/#34452)。共性结论:

> **"无论内部发生什么,用户最终一定收到一条有意义的消息"被当作硬不变量**,由一个统一收敛点兜住所有异常终态。openclaw 在 reply-runner 永远 `return {kind:"final"}` 并显式检测空响应;hermes 用 gateway 末端 `_normalize_empty_agent_response` 无条件兜底,审批超时/拒绝/推送失败统一转 BLOCKED 文本("Silence is not consent")。

| 维度 | echo-agent 现状 | openclaw | hermes-agent |
|---|---|---|---|
...
绝不允许"回合结束但用户什么都没收到"。对齐 hermes `_normalize_empty_agent_response`。
