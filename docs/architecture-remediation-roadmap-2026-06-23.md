...
- 参照系：同级项目 `openclaw`（`SECURITY.md` 已固化 trusted-operator 威胁模型）、`hermes-agent`（同为 Python 自托管 agent，含 gateway）。
...
这一选择与 `openclaw/SECURITY.md` 完全一致，其原文将自身定义为：
...
OpenClaw 的 "What Usually Is Not a Security Bug" 列表进一步明确：本机 shell/脚本执行、操作者自装的插件、多敌对用户共用一个 gateway 期望隔离、文档已建议规避的公网暴露——**都不算安全漏洞**。本路线图直接采用同一把标尺。
...
- **C 类·设计取舍（不改，明确标注）**：trusted-operator 模型下本就是有意为之的本机能力，按 OpenClaw 标尺不算漏洞。**明确标注为非问题，避免被后续误当 bug 修。**
...
> 注意边界：A 类主线一的 SkillReviewer **不是** C 类。区别在于它让注入内容**绕过了本应存在的 gate/扫描边界**自动落盘生效——这正好落在 OpenClaw 认定算漏洞的 "policy/approval/tool-boundary bypass" 一侧。
...
- SECURITY.md：写入第 1 节威胁模型 + B 类加固清单 + "什么不算漏洞"（照 OpenClaw 结构）。
